/* ========================================
   ...
   historial.js — Con paginación "cargar más"
======================================== */
...
function renderHistorial() {
  ...
  const visibleCount = (_historialPage + 1) * HISTORIAL_PAGE_SIZE;
  ...
  if (hasMore) {
    tbody.innerHTML += `<tr><td colspan="7" class="py-4 text-center"><button onclick="cargarMasHistorial()" class="px-6 py-2 bg-blue-100 text-blue-700 rounded-lg text-sm font-semibold hover:bg-blue-200 transition-colors"><i class="fas fa-chevron-down mr-1"></i>Cargar más (${remaining} restantes)</button></td></tr>`;
  }
  ...
  if (mob) {
    ...
    /* Botón "cargar más" en mobile */
    if (hasMore) {
      html += `<div class="text-center py-4"><button onclick="cargarMasHistorial()" class="px-6 py-2.5 bg-blue-100 text-blue-700 rounded-xl text-sm font-semibold hover:bg-blue-200 transition-colors"><i class="fas fa-chevron-down mr-1"></i>Cargar más (${remaining})</button></div>`;
    }
    mob.innerHTML = html;
    initSwipeCards();
  }
...
function filtrarHistorial() { _historialPage = 0; persistHistoryFilters(); renderHistorial(); }
