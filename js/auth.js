...
function showAuthSuccess(msg) {
  const el = $('auth-success'); el.textContent = msg; el.classList.remove('hidden');
  $('auth-error').classList.add('hidden');
}
...
async function doLogin() {
  ...
  try {
    ...
    btn.disabled = false; btn.innerHTML = '<i class="fas fa-sign-in-alt mr-2"></i>Iniciar sesión';
  }
}
...
async function doRegister() {
  ...
  try {
    ...
    btn.disabled = false; btn.innerHTML = '<i class="fas fa-user-plus mr-2"></i>Crear cuenta gratis';
  }
}
...
async function doResetPassword() {
  ...
  try {
    ...
    if (error) throw error;
    showAuthSuccess('¡Listo! Revisá tu email para restablecer tu contraseña.');
...
async function doLogout() {
  ...
  allData = []; dbCentros = []; dbMetodos = []; currentUserId = null;
