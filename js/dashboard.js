...
import { safeNumber, formatearNumero, localMesStr, getMesKey, uniqueSorted, destroyChart, escapeHtml, escapeAttr, aggregateBy } from './utils.js';
...
export function renderDashboard() {
  ...
  const centros = uniqueSorted(S.allData.map(g=>g.Centro));
  ...
  const conceptos = uniqueSorted(S.allData.map(g=>g.Concepto));
...
export function renderEvolucionCentro() {
  ...
  const d=S.allData.filter(g=>g.Centro===c), pm=aggregateBy(d,g=>getMesKey(g.Fecha)||'?'), ms=Object.keys(pm).sort(), vs=ms.map(m=>pm[m]);
  ...
  S.charts.evolucionCentro = new Chart(cv, { type:'bar', data:{ labels:ms, datasets:[{ label:c.substring(0,24), data:vs, backgroundColor:'rgba(16,185,129,.75)', borderColor:'#10b981', borderWidth:1, borderRadius:6, maxBarThickness:40 }]}, options:{ responsive:true, plugins:{legend:{display:true}}, scales:{y:{ticks:{callback:v=>'$'+(v/1e3).toFixed(0)+'k'}}}}});
}
...
export function renderEvolucionConcepto() {
  ...
  cv.style.display='block';
  const d=S.allData.filter(g=>g.Concepto===c), pm=aggregateBy(d,g=>getMesKey(g.Fecha)||'?'), ms=Object.keys(pm).sort(), vs=ms.map(m=>pm[m]);
  ...
  S.charts.evolucionConcepto = new Chart(cv, { type:'bar', data:{ labels:ms, datasets:[{ label:c.substring(0,20), data:vs, backgroundColor:'rgba(139,92,246,.75)', borderColor:'#8b5cf6', borderWidth:1, borderRadius:6, maxBarThickness:40 }]}, options:{ responsive:true, plugins:{legend:{display:true}}, scales:{y:{ticks:{callback:v=>'$'+(v/1e3).toFixed(0)+'k'}}}}});
}
